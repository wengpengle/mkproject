...
class RaisesController extends Controller{
    ...
    public function raises_add(){
        if(Request::isMethod('post')){
            ...
            $arr['parent_id']=Request::input('cp_id');
            $arr['cp_time']=date('Y-m-d H:i:s',time());
            $arr['model_id']=1;
            $result=DB::table('course_part')->insert($arr);
             $son_cp_id=DB::insertGetId();
            if($result){
                $data['son_cp_id']=$son_cp_id;
            // 封面上传
                #上传文件的后缀.
            ...
            // 视频上传
                ...
                $res=DB::table('raises')->insert($data);
                if($res){
    ...
    public function raises_list(){
    	$arr=DB::table('raises')
    			 ...
    			 ->leftjoin('course_part','raises.son_cp_id','=','course_part.cp_id')
